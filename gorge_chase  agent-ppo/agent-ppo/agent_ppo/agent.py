...
class Agent(BaseAgent):
    ...
    def observation_process(self, env_obs):
        ...
        # Store current env_obs for attention recording / 存储当前env_obs用于注意力记录
    ...
    def predict(self, list_obs_data):
        ...
        logits, value, prob, attention_weights = self._run_model(feature, legal_action)
        ...
        return [
            ActData(
                ...
                prob=list(prob),
                value=value,
                attention_weights=attention_weights,  # Add attention weights / 添加注意力权重
            )
        ]
    ...
    def _run_model(self, feature, legal_action):
        """Run model inference, return logits, value, prob, and attention_weights.

        执行模型推理，返回 logits、value、动作概率和注意力权重。
        """
        self.model.set_eval_mode()
        obs_tensor = torch.tensor(np.array([feature]), dtype=torch.float32).to(self.device)

        with torch.no_grad():
            # Get attention weights from model / 从模型获取注意力权重
            result = self.model(obs_tensor, inference=True)
            if len(result) == 3:
                logits, value, attention_weights = result
            else:
                logits, value = result
        ...
        # Convert attention weights to numpy if available / 转换注意力权重为numpy
        attention_weights_np = None
        if attention_weights is not None:
            attention_weights_np = attention_weights.cpu().numpy()[0]  # [num_groups]
        ...
        return logits_np, value_np, prob, attention_weights_np
